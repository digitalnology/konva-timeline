...
# @digitalnology/konva-timeline

`@digitalnology/konva-timeline` is a TypeScript ReactJS library that uses `konva` and `react-konva` to render a timeline component using canvas.
...
```
npm i @digitalnology/konva-timeline
```
...
Import the `KonvaTimeline` component from `@digitalnology/konva-timeline` library:

```
import { KonvaTimeline } from "@digitalnology/konva-timeline";
```
